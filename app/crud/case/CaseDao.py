...
class CaseDao(BaseCrud):
    ...
    # 插入数据
    @classmethod
    def insert_case(cls, project_id: int, title: str, name: str, description: str, group_name: str, header: str,
                    owner: str, path: str,
    ...
    # 更新数据
    def update_case(cls, cases_id: int, title: str, name: str, description: str, group_name: str, header: str,
                    owner: str, path: str,
                    param_in: str, param_out: str, example_param_in: str, example_param_out: str, user: dict) -> None:
        """
        更新造数场景
    ...
    @classmethod
    def get_all_cases(cls, user: dict, page: int = 1, limit: int = 10, show: str = None, project_id: int = None,
                      case_id: int = None):
        """
        用例列表展示
        :param user: 用户数据
        ...
        with Session() as session:
            ...
            is_collection = case_([(collection_.del_flag == DeleteEnum.no.value, True)], else_=False).label(
                "collection")
    ...
    def case_detail_by_id(cls, id: int, user: dict = None):
        ...
        """
        with Session() as session:
            case = session.query(DataFactoryCases.owner, DataFactoryCases.group_name, DataFactoryCases.description,
                                 DataFactoryCases.create_name, DataFactoryCases.create_time,
                                 DataFactoryCases.update_time,
                                 DataFactoryCases.id, DataFactoryCases.path, DataFactoryCases.project_id,
                                 DataFactoryCases.name, DataFactoryCases.example_param_in,
                                 DataFactoryCases.example_param_out,
                                 DataFactoryCases.param_in, DataFactoryCases.param_out, DataFactoryCases.title,
                                 DataFactoryProject.project_name, DataFactoryProject.git_project,
                                 DataFactoryProject.directory).join(
                DataFactoryCases, DataFactoryCases.project_id == DataFactoryProject.id).filter(
                DataFactoryCases.id == id, DataFactoryCases.del_flag == 0).first()
            if user:
    ...
    def case_detail_by_method(cls, name: str):
        ...
        """
        with Session() as session:
            case = session.query(DataFactoryCases.id.label("cases_id"), DataFactoryCases.path,
                                 DataFactoryCases.project_id, DataFactoryCases.name.label("method"),
                                 DataFactoryProject.git_project.label("project"), DataFactoryProject.directory). \
    ...
    def case_group_summary(cls):
        """统计各业务线数量"""
        with Session() as session:
            group_num = session.query(DataFactoryCases.group_name.label("name"),
                                      func.count(DataFactoryCases.id).label("value")) \
    ...
    def collect_weekly_data(cls, start_time: datetime, end_time: datetime):
        ...
        with Session() as session:
            ...
            weekly_data = session.query(date.label("date"), func.count(DataFactoryCases.id).label("sum")) \
                .filter(DataFactoryCases.del_flag == 0,
                        DataFactoryCases.create_time.between(start_time.strftime("%Y-%m-%d 00:00:00"),
...
class CaseParamsDao(BaseCrud):
    ...
    def deleta_cases_params(cls, id: int, user: dict):
        ...
        """
        with Session() as session:
            param_result = session.query(DataFactoryCasesParams) \
                .filter(DataFactoryCasesParams.id == id,
    ...
    def get_params_detail(cls, out_id: str):
        ...
        if param_result is None:
            raise BusinessException("参数组合数据不存在！")
        return param_result
