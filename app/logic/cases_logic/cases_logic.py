...
from app.constants.enums import CallTypeEnum, RunStatusEnum
...
def run_logic(body: RunBody, call_type: CallTypeEnum, user: dict):
    ...
    try:
        ...
        run_status = RunStatusEnum.success.value if run_data.get('responseCode') == 0 or run_data.get('code') == 200 or\
                                                    run_data.get('code') == 0 else RunStatusEnum.exception.value

        end_time = time.perf_counter()
        cost = "%.2fs" % (end_time - start_time)
        LogDao.add(body.cases_id, body.requests_id, body.project_id, json.dumps(body.params, ensure_ascii=False),
                   json.dumps(run_data, ensure_ascii=False),
                   run_status=run_status, call_type=call_type, run_log=run_log, user=user)
        return dict(actual_request=body.params, actual_response=run_data, result=run_status,
                    requests_id=body.requests_id, cost=cost, log=run_log)
    except:
        import traceback
        run_status = RunStatusEnum.fail.value
        run_log = f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]: 脚本执行失败 -> 脚本目录：{body.project}/" \
                  f"{body.directory}/" \
                  f"{body.path}.py，执行{body.method}函数失败，kwargs：{body.params}，报错信息：\n{str(traceback.format_exc())} "
        LogDao.add(body.cases_id, body.requests_id, body.project_id, json.dumps(body.params, ensure_ascii=False),
                   run_param_out=None,
                   run_status=run_status, call_type=call_type, run_log=run_log, user=user)
        end_time = time.perf_counter()
