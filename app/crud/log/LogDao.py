...
class LogDao(BaseCrud):
    ...
    def get_all_logs(cls, user: dict, page: int = 1, limit: int = 20, group: str = None, project: str = None,
        ...
        with Session() as session:
            ...
            log = session.query(DataFactoryRunLog.requests_id, DataFactoryRunLog.run_param_in,
                                DataFactoryRunLog.run_param_out,
                                ...
                                DataFactoryRunLog.create_name, DataFactoryRunLog.create_time,
                                DataFactoryRunLog.create_id). \
    ...
    def run_status_summary(cls):
        ...
        with Session() as session:
            run_type = case(
                whens=[(DataFactoryRunLog.run_status == 0, '成功'), (DataFactoryRunLog.run_status == 1, '异常')],
                else_='失败').label("name")
            run_status_sum = session.query(run_type, func.count(DataFactoryRunLog.run_status).label("value")).group_by(
                DataFactoryRunLog.run_status)
    ...
    def call_type_summary(cls):
        """统计各调用方式数量"""
        with Session() as session:
            type = case(whens=[(DataFactoryRunLog.call_type == '0', '平台调用'),
                               (DataFactoryRunLog.call_type == '1', '外链调用')], else_='RPC调用').label("name")
    ...
    def collect_weekly_data(cls, start_time: datetime, end_time: datetime):
        ...
        with Session() as session:
            ...
            filter_list = [DataFactoryRunLog.create_time.between(start_time.strftime("%Y-%m-%d 00:00:00"),
    ...
    def fill_data(cls, start_time: datetime, end_time: datetime, call_data: dict, case_data: dict):
        ...
        while start <= end_time:
            ...
            weekly_data.append(dict(date=date, call_count=call_data.get(date).get('count') if date in call_data else 0,
                                    ...
                                    exception_count=call_data.get(date).get(
                                        'exception_count') if date in call_data else 0,
